...
start_server() {
    ...
    cd "$PROJECT_DIR"
    uv run mcp-server-datahub --transport "$transport" >/dev/null 2>&1 &
    SERVER_PID=$!
...
run_smoke_check() {
    ...
    if uv run python "$SMOKE_CHECK" ${extra[@]+"${extra[@]}"} ${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}; then
        MODE_RESULTS+=("PASS")
        echo "  ➜ $mode_name: PASS"
    else
        MODE_RESULTS+=("FAIL")
...
cd "$PROJECT_DIR"
uv run fastmcp run src/mcp_server_datahub/__main__.py:create_app --transport http >/dev/null 2>&1 &
SERVER_PID=$!
wait_for_server "$HEALTH_URL"
