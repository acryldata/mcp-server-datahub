...
def test_add_owners_to_multiple_datasets(mock_datahub_client):
...
def test_add_owners_with_nonexistent_owner(mock_datahub_client):
...
def test_remove_owners_from_multiple_datasets(mock_datahub_client):
...
def test_remove_owners_with_ownership_type(mock_datahub_client):
...
def test_remove_owners_with_nonexistent_owner(mock_datahub_client):
...
def test_remove_owners_mutation_returns_false(mock_datahub_client):
    """Test handling of mutation returning false."""
    owner_urns = ["urn:li:corpuser:test"]
    entity_urns = ["urn:li:dataset:test"]
